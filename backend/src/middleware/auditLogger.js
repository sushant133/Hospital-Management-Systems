...
/**
 * How deep to walk before giving up. Clinical documents nest a few levels
 * (encounter → admission → transfers → …); far beyond that is a structure the
 * trail has no business copying whole.
 */
const MAX_DEPTH = 12;

/** Arrays longer than this are summarised — an audit row is not a backup. */
const MAX_ARRAY_LENGTH = 200;

/**
 * Convert an arbitrary value into something safe to store on an audit row.
 *
 * Guards against cycles and depth explicitly. A Mongoose document carries
 * internal back-references (`$__parent` and friends) that form a cycle, so a
 * naive recursive walk overflows the stack — and because the trail is written
 * after the response has flushed, that crash lands outside any request's error
 * handling and takes the process down with it.
 */
function normalizeValue(value, depth = 0, seen = new WeakSet()) {
  ...
  if (typeof value !== 'object') {
    // Functions have no place on an audit row; primitives pass through.
    return typeof value === 'function' ? undefined : value;
  }

  // File uploads and binary fields must never be inlined byte by byte.
  ...
  try {
    // Collapse Mongoose documents and subdocuments to plain data first, which
    // drops the internal parent links that made the graph cyclic.
    if (typeof value.toObject === 'function') {
      return normalizeValue(value.toObject({ depopulate: true }), depth, seen);
    }

    if (Array.isArray(value)) {
      const items = value.slice(0, MAX_ARRAY_LENGTH).map((item) => normalizeValue(item, depth + 1, seen));
    ...
    for (const [key, nested] of Object.entries(value)) {
      out[key] = REDACTED_FIELDS.has(key)
        ? REDACTION_PLACEHOLDER
        : normalizeValue(nested, depth + 1, seen);
    }
    return out;
  } finally {
    // Released so a value legitimately repeated in two branches is not
...
export function audit({
  ...
  return function auditMiddleware(req, res, next) {
    ...
    res.on('finish', () => {
      /**
       * Everything below runs after the response has flushed, so it sits
       * outside Express's error handling: an exception here is uncaught and
       * takes the process down. Building the diff must therefore be as
       * defensive as writing the row already is — losing one audit entry is
       * bad, crashing the server mid-clinic is far worse.
       */
      try {
        writeEntry();
      } catch (error) {
        console.error('[audit] failed to build audit entry:', error.message, {
          action,
          resourceType,
