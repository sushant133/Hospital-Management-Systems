...
export const listQuery = z.object({
   ...
   * Page size.
   *
   * The ceiling is 500, not 100, because a large share of callers are not
   * paging at all — they are loading a reference list into a picker: every drug
   * in the formulary, every bed in a ward, every staff member for a roster.
   * A cap of 100 rejected those outright with a 422.
   *
   * It stays capped because an unbounded limit is a denial-of-service waiting
   * for one impatient client. Where a list genuinely exceeds 500, `meta.truncated`
   * tells the caller so a picker can say "refine your search" rather than
   * silently omitting the drug someone is looking for.
   */
  limit: z.coerce
    .number()
    .int()
    .min(1)
    .max(500, 'Page size cannot exceed 500. Use search or paging for larger sets.')
    .default(20),
