...
import MedicalHistoryTab from '../components/MedicalHistoryTab.jsx';
import MergeModal from '../components/MergeModal.jsx';
import PatientLabTab from '../../lab/components/PatientLabTab.jsx';
import PatientRadiologyTab from '../../radiology/components/PatientRadiologyTab.jsx';
import { useAuth } from '../../../hooks/useAuth.js';
import { MODULES } from '../../../utils/permissions.js';
import { ageFrom, formatDate, fullName, initials, titleCase } from '../../../utils/format.js';
import {
...
const TABS = [
  ...
  { key: 'radiology', label: 'Radiology', require: [MODULES.RADIOLOGY_ORDERS, 'view'] },
];

const STATUS_TONES = { active: 'success', inactive: 'neutral', deceased: 'danger', merged: 'purple' };
...
export function PatientDetailPage() {
  ...
  const canMerge = can(MODULES.PATIENTS, 'merge');
  const canInvitePortal = can(MODULES.PORTAL, 'invite');
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <Button variant="secondary" onClick={() => navigate(`/patients/${id}/timeline`)}>
              Timeline
            </Button>
            {canInvitePortal && patient.isActive && (
              <Button
                variant="secondary"
                onClick={async () => {
                  const email = window.prompt('Portal email', patient.email || '');
                  const password = window.prompt('Temporary password (8+ chars, mixed case, digit)');
                  if (!email || !password) return;
                  try {
                    const response = await patientsApi.invitePortal(id, { email, password });
            ...
            {canMerge && patient.isActive && patient.status !== 'merged' && (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Card>
            ...
            <dl className="divide-y divide-slate-100">
              ...
              {patient.status === 'merged' && patient.mergedInto && (
                <DataRow
                  label="Merged into"
                  value={
                    <Link to={`/patients/${patient.mergedInto._id ?? patient.mergedInto}`} className="text-brand-600">
                      Surviving chart
                    </Link>
                  }
                />
      ...
      {tab === 'radiology' && <PatientRadiologyTab patient={patient} />}
          ...
          {!visitsLoading && visits.length === 0 && !visitsError ? (
            ...
            <Table>
              ...
              <TBody>
                ...
                {!visitsLoading &&
                  visits.map((visit) => (
                    <TR key={visit._id} onClick={() => navigate(`/encounters/${visit._id}`)}>
                      <TD>
                        <Link
                          to={`/encounters/${visit._id}`}
