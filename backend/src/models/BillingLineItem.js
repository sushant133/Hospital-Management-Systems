 ...
 * originally: charges accrue continuously throughout an encounter and
...
const billingLineItemSchema = new Schema(
  {
    // Every financial row carries both patient and encounter.
    patientId: {
    ...
    /** GST / VAT on this line. Invoice tax is the sum of line tax when set. */
...
billingLineItemSchema.pre('validate', function computeLineTotal(next) {
  ...
  const taxPercent = Number(this.taxPercent ?? 0);
  this.taxAmount = Math.round(this.lineTotal * taxPercent) / 100;
  next();
});
