...
export async function connectDatabase({ retries = 5, delayMs = 3000 } = {}) {
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      // Probe transaction support at boot, not at the counter. An operator who
      // has started a standalone mongod needs to learn that payments and
      // dispensing will be refused now, rather than from a failed payment.
      const { reportTransactionSupport } = await import('../utils/transaction.js');
      await reportTransactionSupport();
