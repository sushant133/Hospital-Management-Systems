...
export const login = asyncHandler(async (req, res) => {
  ...
  if (user && isLocked(user)) {
    await recordAudit({
      ...
      userRole: user.role,
      resourceId: user._id,
      reason: 'Account locked after repeated failed passwords',
    });
    throw ApiError.forbidden(
      'This account is temporarily locked after too many failed sign-in attempts. Try again later or contact an administrator.',
      { code: 'ACCOUNT_LOCKED' },
    );
  }
  ...
  if (!passwordMatches) {
    const lock = registerFailedLogin(user, {
      maxAttempts: config.auth.maxFailedLogins,
      lockMinutes: config.auth.lockMinutes,
    });
    user.failedLoginCount = lock.failedLoginCount;
    ...
    await recordAudit({
      ...
      action: 'login_failed',
      outcome: 'failure',
      statusCode: lock.justLocked ? 403 : 401,
      userId: user._id,
      userName: `${user.firstName} ${user.lastName}`.trim(),
      userRole: user.role,
      resourceId: user._id,
      reason: lock.justLocked
        ? `Account locked after ${lock.failedLoginCount} failed passwords`
        : 'Incorrect password',
    });
  ...
  Object.assign(user, registerSuccessfulLogin());
